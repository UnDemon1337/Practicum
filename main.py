...
from task3.table_manager import play as play_table_manager
...
def display_main_menu():
    ...
    print(" 3. Работа с таблицами")  # Добавленная строка
    print(" 0. Выход")
    print("=" * 60)
...
def run_game(choice):
    ...
    games = {
        1: lambda: game1_bulls_and_cows.play(),
        2: lambda: game2_tic_tac_toe.play(),
...
def run_table_manager():
    """Запускает менеджер таблиц"""
    clear_screen()
    play_table_manager()
...
def games_section():
    ...
    while True:
        clear_screen()
        display_games_menu()
        choice = get_user_choice(0, 3, "Введите номер игры (0-3): ")
...
def main():
    ...
    while True:
        clear_screen()
        display_main_menu()
        choice = get_user_choice(0, 3, "Введите номер задачи (0-3): ")  # Изменено на 3
        ...
        if choice == 0:
            ...
            run_calculator()
        elif choice == 3:  # Добавленный блок
            run_table_manager()
